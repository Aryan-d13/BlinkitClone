      ...
      <div className="bg-slate-900 text-slate-300 py-1.5 px-4 text-xs">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-semibold tracking-wide uppercase">
              {SITE_CONTENT.navigation.announcementBadge}
            </span>
            <span className="text-slate-300">
              <strong className="text-white font-semibold">${remainingCredit.toFixed(2)}</strong> {SITE_CONTENT.navigation.announcementSuffix}
            </span>
          </div>

          <div className="hidden sm:flex items-center gap-4 text-slate-400 text-[11px]">
            <span className="flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-emerald-400" /> Order by {SITE_CONTENT.brand.cutoffTime}
            </span>
            <span className="text-slate-700">|</span>
            <span>Delivery: {SITE_CONTENT.brand.deliveryWindow}</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            <Link href="/" className="flex items-center gap-3 group">
              <div className="w-9 h-9 rounded-xl bg-slate-900 text-white flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform duration-300">
                <UtensilsCrossed className="w-4.5 h-4.5 text-emerald-400 stroke-[2.2]" />
              </div>
              <div>
                <h1 className="text-lg font-bold text-slate-900 tracking-tight leading-none">
                  {SITE_CONTENT.brand.brandFirst}<span className="text-emerald-700">{SITE_CONTENT.brand.brandSecond}</span>
                </h1>
                    ...
                    >
                      ...
                      <div className="flex-1 text-xs">
                        <div className="font-semibold text-slate-800 flex items-center justify-between">
                          ...
                          {selectedAddress.id === addr.id && <CheckCircle className="w-3.5 h-3.5 text-emerald-700" />}
                        </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              {wishlist.length > 0 && (
                <span className="absolute top-1.5 right-1.5 w-3.5 h-3.5 bg-emerald-700 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                  {wishlist.length}
                </span>
              )}
            </Link>
            ...
            >
              ...
              <span className="tabular-nums">
                {totalCartItemCount === 0 ? 'Cart' : `$${cartSubtotal.toFixed(0)}`}
              </span>
